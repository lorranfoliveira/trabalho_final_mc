...
@testset verbose = true "Analise" begin
    ...
    @testset "Elementos" begin
        ...
        @test Analise.angulo(elementos[9]) ≈ pi
    ...
    @testset "Estrutura" begin
        ...
        @test Analise.k_estrutura_1(estrutura) ≈ k
        @test Analise.k_estrutura_2(estrutura) ≈ k

        @test Analise.deslocamentos(estrutura, 1) ≈ Analise.deslocamentos(estrutura, 2) ≈ [0.0, 0.0, 4.761904761904763e-5, -0.00019817906943235845, 2.3809523809523814e-5, -7.528001090665546e-5, 0.0, 0.0, 0.0002696076408609297, -0.0002696076408609298, 0.00012675049800378677, -7.528001090665545e-5]

        @test Analise.forcas_internas(estrutura) ≈ [3.3340206284604434, 0.0012720448909134228, -1.6664023403833994, 7.0710761922049326, -1.6665674915761208, -4.999137587831104, 0.00018544477509152557, -7.07105622101196, -9.999339096309345]
    end
